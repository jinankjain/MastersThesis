...
\SetupFloatingEnvironment{listing}{name=Listings}
...
%\noindent\enquote{\itshape template}\bigbreak
...
% Accountability and privacy are important for current internet. 
% however supporting both at the same time is conflicting and why its challenging
In today's world most of us would agree that accountability and privacy are both valuable, but current Internet provides little support for either. Packets on the current cannot be attributed to their source, and communication on the Internet cannot be kept private. Furthermore even if the current Internet could be modified to support those two properties providing both is difficult due to their conflict nature. Hence most Future Internet Architectures have focused on one at the expense of other.

Accountable and Private Network Architecture (APNA) shows that it is possible to find the sweet spot between accountability and privacy using ISP as trusted third parties and using Ephemeral addresses instead of IP addresses for communication. Unfortunately, APNA does not provide a concrete implementation thus it lacks some details to realize it inside an Internet architecture. This works focus on extending the APNA protocol with all the missing information and implementing it inside a future internet architecture called Scalability, Control, and Isolation On Next-Generation Networks(SCION). It also explores different ways to integrate APNA with SCION and the associated trade-off with each approach. Finally this work deploys the implementation on SCIONLab, which is a testbed for SCION and evaluates the performance overhead of the APNA implementation against that of a vanilla SCION
\end{abstract}
...
\dedicatory{Dedicated to my family, friends, colleagues and ETH} 
...
\include{Chapters/APNAMS}
\include{Chapters/ApnaOverlay}
\include{Chapters/AddressFamily}
\include{Chapters/ApnaService}
\include{Chapters/ScionLab} 
\include{Chapters/PerformanceEval}
